...
def deploy_mocks(decimals=18, initial_value=2000):
    ...
    link_token = LinkToken.deploy({"from": account, "gas_price": gas_price})

    print("Deploying Mock Price Feed...")
    mock_price_feed = MockV3Aggregator.deploy(
        decimals, initial_value, {"from": account, "gas_price": gas_price}
    )
    ...
    mock_vrf_coordinator = VRFCoordinatorMock.deploy(
        link_token.address, {"from": account, "gas_price": gas_price}
    )
    print(f"Deployed to {mock_vrf_coordinator.address}")

    print("Deploying Mock Oracle...")
    mock_oracle = MockOracle.deploy(link_token.address, {"from": account, "gas_price": gas_price})
...
def fund_with_link(
    contract_address, account=None, link_token=None, amount=1000000000000000000, gas_price=None
):
    ...
    tx = interface.LinkTokenInterface(link_token).transfer(
        contract_address, amount, tx_params
    )
